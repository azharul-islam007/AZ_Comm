...
def adapt_tensor_to_system_dims(tensor, target_type="compressed"):
    """Adapt tensor to system dimensions with lazy import to avoid circular references"""
    # Import here instead of at the top to avoid circular imports
    import importlib
    physical_semantic_integration = importlib.import_module('physical_semantic_integration')

    # Get dimensions from registry
    dims = physical_semantic_integration.DimensionRegistry().get_dims()

    # Select target dimension
    if target_type == "original":
        target_dim = dims['original']
    elif target_type == "compressed":
        target_dim = dims['compressed']
    elif target_type == "latent":
        target_dim = dims['latent']
    else:
        raise ValueError(f"Unknown dimension type: {target_type}")

    # Use existing ensure_tensor_shape with correct dimensions
    return ensure_tensor_shape(tensor, expected_dim=2, target_feature_dim=target_dim)
...
def enhance_critical_terms(reconstructed_text):
    """Ensure critical parliamentary terms are correctly spelled"""
    # Look for near-matches of important terms
    critical_terms = {
        "agenda": ["agxnda", "aginda", "aegnda", "aleeda", "agwnda"],
        "parliament": ["parlxment", "parlmnt", "paliament", "parliement", "paleament"],
        "commission": ["comission", "commiwsion", "commzssion", "komishn"],
        "council": ["counzil", "cuoncil", "cowncil", "couvcil", "konzil"],
        "meeting": ["meetin", "ieetpng", "meating", "mitink", "meetyng"]
    }

    result = reconstructed_text
    for correct, variants in critical_terms.items():
        # First check exact matches of corrupted forms
        for variant in variants:
            if variant in result.lower():
                ...
                if idx >= 0:
                    before = result[:idx]
                    after = result[idx + len(variant):]
                    # Match capitalization
                    if result[idx].isupper():
                        correct_term = correct.capitalize()
                    else:
                        correct_term = correct
                    result = before + correct_term + after

        # Now check fuzzy matches (words that are close but not exact matches)
        words = result.split()
        for i, word in enumerate(words):
            if len(word) >= 4:  # Only check words of reasonable length
                for term in [correct]:  # Check against the correct form
                    similarity = difflib.SequenceMatcher(None, word.lower(), term).ratio()
                    if 0.6 < similarity < 0.9:  # Close but not exact match
                        # Likely corruption - replace it
                        if word[0].isupper():
