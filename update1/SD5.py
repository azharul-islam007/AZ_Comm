...
OPENAI_API_KEY = "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"
...
def basic_text_reconstruction(noisy_text):
    """Simple rule-based text reconstruction for when API is unavailable"""
    ...
    for word in words:
        ...
        if len(word) > 3:
            ...
            if has_strange_pattern:
                # Just note it in debug - would need a dictionary in a real system
                logger.debug(f"Unusual word pattern detected: {word}")
...
def api_reconstruct_with_semantic_features(noisy_text, context="", rl_agent=None, budget_remaining=1.0,
    ...
    """
    Enhanced version of API reconstruction with semantic features for better RL decisions.
    ...
    if not openai_available or not openai_client:
        ...
        reconstructed = basic_text_reconstruction(noisy_text)
    ...
    if rl_agent is not None:
        ...
        # Select action: 0=basic, 1=GPT-3.5, 2=GPT-4
...
def run_enhanced_pipeline(num_samples=50, noise_level=0.1, noise_type='gaussian',
    ...
    """
    Run the complete enhanced semantic communication pipeline with:
    1. VAE-based advanced compression
    2. Semantic perceptual loss for better reconstruction
    3. Content-adaptive coding for better channel protection

    Args:
        num_samples: Number of samples to process
        noise_level: Noise level to apply (0-1)
        noise_type: Type of noise ('gaussian', 'burst', 'dropout')
        use_api_pct: Percentage of samples to use API for (0-1) - used only without RL
        comparison_mode: Whether to compare embedding-based vs direct text reconstruction
        use_self_supervised: Whether to use self-supervised learning
        use_semantic_loss: Whether to use semantic perceptual loss
        use_vae_compression: Whether to use VAE-based compression
