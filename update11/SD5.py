...
class AdvancedRLAgent(nn.Module):
    ...
    def select_action(self, state, budget_remaining, force_basic=False, corruption_level=None):
        """Enhanced action selection with more aggressive API usage and better corruption detection"""
...
def api_reconstruct_with_semantic_features(noisy_text, context="", rl_agent=None, budget_remaining=1.0,
    ...
    # 1. Check corruption level threshold
    if corruption_level > 0.25:  # More permissive threshold (was 0.35)
        should_use_api = True
        # Use GPT-4 for more severely corrupted text
        if corruption_level > 0.4 and budget_remaining > 0.1:
            api_model = "gpt-4-turbo"

    # 2. Check if KB confidence is low but non-zero (KB made an attempt but not confident)
    if kb_applied and kb_confidence > 0 and kb_confidence < 0.7:
        should_use_api = True

    # 3. Check for critical parliamentary terms that might need better reconstruction
    ...
    # 4. Use RL agent decision if available
