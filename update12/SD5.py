    ...
    def select_action(self, state, budget_remaining, force_basic=False, corruption_level=None):
        ...
        with torch.no_grad():
            ...
            if torch.isnan(masked_probs).any():
                logger.warning("NaN detected in masked probabilities, using default values")
                masked_probs = torch.tensor([0.6, 0.3, 0.1], device=masked_probs.device)
                masked_probs *= mask  # Apply mask again
...
def basic_text_reconstruction(noisy_text, use_kb=True):
    ...
    for word in words:
        ...
        # Don't replace if similarity is too low - THIS IS THE KEY IMPROVEMENT
    ...
    if result == noisy_text:
        # No changes made, try one more approach
        known_fixes = {
            "ruiye": "quite",
            "dwne": "done",
            "shuml": "shall"
        ...
        # Apply direct word replacement
        for old, new in known_fixes.items():
            if old in result:
                result = result.replace(old, new)
                changes_made = True
                logger.info(f"Applied direct replacement: {old} -> {new}")

    # Final verification
    if changes_made and result == noisy_text:
        logger.warning(f"Changes were reported but result is unchanged: '{noisy_text}'")
...
def api_reconstruct_with_semantic_features(noisy_text, context="", rl_agent=None, budget_remaining=1.0,
    ...
    if request_key in _api_recovery_attempts:
        ...
        if _api_recovery_attempts[request_key] > 2:
            # Too many recovery attempts, use KB reconstruction instead of basic
            logger.warning("Too many API recovery attempts, trying KB reconstruction")
            kb = get_or_create_knowledge_base()
            kb_result = kb.kb_guided_reconstruction(noisy_text)
            if kb_result != noisy_text:
                return kb_result, 0, 0
            # If KB also fails, then try basic
    ...
    api_model = "gpt-3.5-turbo"  # Default model

    # 1. Check corruption level with lower thresholds for more API use
    if corruption_level > 0.10:  # Extremely permissive threshold
        should_use_api = True
        # Use GPT-4 for more severely corrupted text with lower threshold
        if corruption_level > 0.25 and budget_remaining > 0.05:  # Even more aggressive
            api_model = "gpt-4-turbo"

    problem_words = ["ruiye", "dwne", "shuml", "kuite", "qute", "qite", "actuall"]
    if any(word in noisy_text.split() for word in problem_words):
        should_use_api = True
        # These specific errors warrant stronger model
        api_model = "gpt-4-turbo"
        logger.info(f"Detected specific problem word, forcing API usage")

    # 2. Check for challenging patterns that indicate need for API even at lower corruption
    challenging_patterns = ['bb', 'bz', 'hz', 'jz', 'oh', 'xj', 'nx', 'wk', 'vb', 'xn', 'qx', 'oj', 'zx']
    if any(pattern in noisy_text.lower() for pattern in challenging_patterns):
        should_use_api = True
        # Use stronger model if pattern indicates difficult corruption
        if sum(pattern in noisy_text.lower() for pattern in challenging_patterns) >= 2 and budget_remaining > 0.05:
            api_model = "gpt-4-turbo"

    # 3. Higher budget = more willingness to use API
    if budget_remaining > 0.7 and not should_use_api:
        # With plenty of budget, be much more willing to use API
        # Use API with probability proportional to budget and corruption
        api_probability = min(0.8, budget_remaining * corruption_level * 4)
        if np.random.random() < api_probability:
            should_use_api = True

    # 4. Check if KB confidence is low but non-zero (KB made an attempt but not confident)
    if kb_applied and kb_confidence > 0 and kb_confidence < 0.7:
        should_use_api = True

    # 5. Check for critical parliamentary terms that might need better reconstruction
    parliamentary_terms = ["Parliament", "Commission", "Council", "Directive", "Regulation",
                           "Quaestors", "Rule", "amendment", "proposal"]
    for term in parliamentary_terms:
        # Check for partial matches that might indicate corrupted important terms
        term_lower = term.lower()
        for word in noisy_text.split():
            # Calculate string similarity
            similarity = difflib.SequenceMatcher(None, word.lower(), term_lower).ratio()
            if 0.6 < similarity < 0.9:  # Close but not exact match
                should_use_api = True
                break

    # 6. Use RL agent decision if available
    if rl_agent is not None:
        try:
            # Prepare state with corruption level
            if semantic_features is not None and hasattr(rl_agent, 'get_enhanced_state'):
                state = rl_agent.get_enhanced_state(corruption_level, len(noisy_text.split()), semantic_features)
            else:
                # Fallback to simple state
                state = torch.tensor([corruption_level, min(1.0, len(noisy_text.split()) / 100),
                                      budget_remaining], dtype=torch.float32)

            # Get action from RL agent
            action, log_prob = rl_agent.select_action(state, budget_remaining,
                                                      force_basic=False,
                                                      corruption_level=corruption_level)

            # Override should_use_api based on RL agent decision
    ...
    # NEW: Budget awareness - if we have lots of budget left, be more willing to use API
    if budget_remaining > 0.8:
        # If we have >80% budget remaining and moderate corruption, use API more freely
        if corruption_level > 0.2:
            should_use_api = True

    # Skip API if not available or shouldn't use
    if not openai_available or not openai_client or not should_use_api:
        if not should_use_api:
            logger.info("[API] Decision made to use non-API reconstruction")
        else:
...
def should_use_api(noisy_text, kb_result=None, kb_confidence=0.0, budget_remaining=1.0, rl_agent=None):
    ...
    Args:
        noisy_text: Corrupted text to analyze
        kb_result: Optional KB reconstruction result
        kb_confidence: Confidence in KB result
        budget_remaining: Remaining API budget fraction
        rl_agent: Optional RL agent for decision making

    Returns:
        Tuple of (should_use_api (bool), model_choice (str), reason (str))
    """
    # Default choices
    should_use = False
    ...
    # Step 3: Check if KB was effective
    kb_effective = False
    if kb_result is not None and kb_result != noisy_text:
        kb_effective = True

    # Step 4: Decision logic

    # Critical corruption cases - use API regardless of KB
    if corruption_info['critical_corruptions'] > 0:
        should_use = True
        reason = "critical_corruptions"
        # Use GPT-4 for name or institution corruptions if budget allows
        if corruption_info['name_corruptions'] > 0 or corruption_info['institution_corruptions'] > 0:
            if budget_remaining > 0.1:
                model = "gpt-4-turbo"

    # High general corruption
    elif corruption_level > 0.3:
        should_use = True
        reason = "high_corruption"

        # If budget is plentiful, use more powerful model
        if budget_remaining > 0.5 and corruption_level > 0.5:
            model = "gpt-4-turbo"

    # KB wasn't effective enough
    elif kb_effective and kb_confidence < 0.7:
        should_use = True
        reason = "low_kb_confidence"

    # Check budget constraints more strictly for non-critical cases
    if should_use and budget_remaining < 0.05:
        # Only allow API for critical corruptions when budget is low
        if not (reason == "critical_corruptions" and budget_remaining > 0.02):
            should_use = False
