    ...
    def calculate_reward(self, metrics, action, cost=0):
        ...
        if action > 0:  # API was used
            ...
            if action == 1:  # GPT-3.5
                ...
                cost_scale = 2.0 * (1.0 + (1.0 - budget_remaining) ** 1.5)  # Progressive scaling
            else:  # GPT-4
                # Similarly for GPT-4 but with different base
                cost_scale = 1.5 * (1.0 + (1.0 - budget_remaining) ** 1.3)  # Progressive scaling
...
def multi_stage_reconstruction(noisy_text, context=None, rl_agent=None, budget_remaining=1.0):
    ...
    Returns:
        Best reconstruction and method used
    """
    ...
    if should_use_api and openai_available and budget_remaining > 0.05:
        ...
        model = "gpt-4-turbo" if corruption_level > 0.6 else "gpt-3.5-turbo"
        ...
        try:
            ...
            if use_ensemble:
                ...
                if len(reconstructions) >= 2:
                    semantic_reconstructed = adaptive_ensemble_voting(corrupted_text, reconstructions, confidences)
