...
def safe_calculate_overall_scores(results):
    """
    A safe version of _calculate_overall_scores that avoids modifying during iteration.
    """
    overall = {}

    # Create copies of dictionaries to prevent modification during iteration
    semantic_metrics = dict(results["semantic_metrics"])
    standard_metrics = dict(results["standard_metrics"])
    domain_metrics = dict(results["domain_metrics"])
    info_metrics = dict(results["information_metrics"])
...
class EnhancedEvaluationFramework:
    ...
    def safe_evaluate_reconstruction(self, original_texts, reconstructed_texts):
        """
        Safe version of evaluate_reconstruction.
        """
        # Handle single text input
        if isinstance(original_texts, str):
            original_texts = [original_texts]
        ...
        # Calculate metrics
        standard_metrics = self._calculate_standard_metrics(original_texts, reconstructed_texts)
        results["standard_metrics"] = dict(standard_metrics)

        semantic_metrics = self._calculate_semantic_metrics(original_texts, reconstructed_texts)
        results["semantic_metrics"] = dict(semantic_metrics)

        domain_metrics = self._calculate_domain_metrics(original_texts, reconstructed_texts)
        results["domain_metrics"] = dict(domain_metrics)

        info_metrics = self._calculate_information_metrics(original_texts, reconstructed_texts)
        results["information_metrics"] = dict(info_metrics)

        # Use the safe calculation function
        results["overall"] = safe_calculate_overall_scores(results)
