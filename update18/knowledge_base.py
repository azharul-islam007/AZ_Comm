...
class SemanticKnowledgeBase:
    ...
    def _initialize_europarl_kb(self):
        ...
        self.term_dict = {"wkulz": "would", "couvsc": "course", "principdas": "principles", "accordancg": "accordance",
                          ...
                          "couggries": "countries", "thdir": "their", "mar": "can","thia": "this","tjis": "this","ghft": "this","matzer": "matter","mafter": "matter","metter": "matter",
                          "agxnda": "agenda","agenta": "agenda","whht": "that","thet": "that","thrt": "that","thwt": "that","thxt": "that"}
    ...
    def _fuzzy_match(self, word, threshold=0.7):
        ...
        # Don't replace common correct words
        ...
        # NEW ADDITION: Special handling for parliamentary terms
        parliamentary_terms = [
            "agenda", "parliament", "commission", "council", "rule",
            "proposal", "procedure", "vote", "quaestor", "meeting"
        ]

        # NEW ADDITION: Check if close to parliamentary term with lower threshold
        for term in parliamentary_terms:
            if len(word) > 3:  # Only for words of reasonable length
                parl_similarity = difflib.SequenceMatcher(None, word.lower(), term.lower()).ratio()
                if parl_similarity > 0.6:  # Lower threshold specifically for parliamentary terms
                    # Get corresponding correction from dictionary if available
                    if term in self.term_dict:
                        return self.term_dict[term], parl_similarity
                    else:
                        return term, parl_similarity
