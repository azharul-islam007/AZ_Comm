...
def initialize_system_enhancements():
    ...
    # 3. Enable contrastive semantic loss
    try:
...
def api_reconstruct_with_semantic_features(noisy_text, context="", rl_agent=None, budget_remaining=1.0,
    ...
    # Try Mini-LLM reconstruction if available and quality thresholds aren't met
    mini_llm_result = noisy_text
    mini_llm_quality = 0.0
    mini_llm_applied = False
    mini_llm_confidence = 0.0

    # Only try Mini-LLM if KB and basic reconstructions aren't sufficient
    if not force_api and (not kb_applied or kb_quality < 0.7) and (not basic_applied or basic_quality < 0.7):
        try:
