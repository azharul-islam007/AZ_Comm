...
def transmit_through_physical_channel(embedding, importance_weights=None, debug=False, use_kb=True, context=None,
    ...
    # Import knowledge base at the beginning to avoid scope issues
    kb = None
    if use_kb:
        try:
            from knowledge_base import get_or_create_knowledge_base
            ...
            logger.debug(f"Knowledge base initialization failed: {e}")
    ...
    # Check if this is a critical sentence containing key parliamentary terms
    is_critical = False
    critical_terms = [
        "Rule", "Parliament", "President", "Commission", "Council",
    ...
    if importance_weights is None and use_kb and kb is not None:
        try:
            # Apply semantic-aware importance weighting
            if hasattr(kb, 'get_importance_weights'):
                importance_weights = kb.get_importance_weights(embedding_np)
    ...
    # Initialize received_embedding with the original to avoid reference before assignment errors
    received_embedding = embedding_np

    # Initial transmission
    max_retries = 2 if is_critical and retry_critical else 0
    retries = 0
    transmission_quality = 0.0

    # Get reference to physical channel (to avoid protected member access warning)
    physical_channel = physical_semantic_bridge._physical_channel

    while retries <= max_retries:
        try:
            # Transmit through physical channel
            # Note: accessing _physical_channel is the intended design pattern in this codebase
            received_embedding = physical_channel.transmit(
                embedding_np, importance_weights, debug=False)

            # Apply post-transmission semantic error recovery
            received_embedding = semantic_error_recovery(
                received_embedding,
                context_embeddings=[original_embedding] if retries > 0 else None,
                kb=kb
            )

            # Evaluate transmission quality
            if is_critical:
                # For critical content, check similarity to original
                transmission_quality = compute_embedding_similarity(original_embedding, received_embedding)

                # If quality is too low, retry with enhanced protection
                if transmission_quality < 0.7 and retries < max_retries:
                    logger.info(f"Critical content detected with low quality ({transmission_quality:.2f}). Retrying...")

                    # Enhance importance weights for retransmission
                    if importance_weights is not None:
                        # Boost all weights by 20%
                        importance_weights = importance_weights * 1.2
                        # Cap at 1.0
                        importance_weights = np.minimum(importance_weights, 1.0)

                    retries += 1
                    continue

            # If we get here, either the quality is acceptable or we've used all retries
            break
            ...
            if retries < max_retries:
                ...
                return embedding  # Return original if all retries fail
