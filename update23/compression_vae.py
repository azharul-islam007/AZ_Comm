...
class EmbeddingCompressorVAE(nn.Module):
    ...
    def calculate_semantic_entropy(self, embedding):
        ...
        entropy = min(1.0, variance / 10.0)  # 10.0 is a normalization factor
    ...
    def get_dynamic_compression_dim(self, embedding, base_ratio=0.6, min_ratio=0.4, max_ratio=0.8):
        ...
        # Scale compression ratio based on entropy
        # Higher entropy = less compression (more dimensions preserved)
        # Lower entropy = more compression (fewer dimensions needed)
    ...
    def compress_with_dynamic_ratio(self, x):
        ...
        with torch.no_grad():
            try:
                ...
                logger.info(
                    f"Dynamic compression: entropy={entropy:.4f}, ratio={dynamic_dim / self.input_dim:.2f}, dim={dynamic_dim}")
...
class EnhancedVAECompressor(nn.Module):
    ...
    def calculate_semantic_entropy(self, embedding):
        ...
        # More sophisticated entropy calculation with multiple components

        # 1. Variance-based component (measures overall variability)
        variance = np.var(embedding_np, axis=1).mean()

        # Use adaptive normalization based on actual data ranges
        # Most embedding values are between -2 and 2, so variance is typically < 1
        variance_component = min(1.0, variance * 5.0)  # Lower factor (5.0 instead of 10.0)

        # 2. Distribution-based component (approximates actual entropy)
        # Use histogram to estimate probability distribution
        try:
            # Flatten the embedding for histogram
            flat_embedding = embedding_np.flatten()
            hist, _ = np.histogram(flat_embedding, bins=20, density=True)
            ...
            dist_entropy = -np.sum(hist * np.log2(hist)) / 4.0  # Normalize by typical max value
            dist_component = min(1.0, dist_entropy)
        except Exception:
            # Fallback if histogram fails
            dist_component = 0.5

        # 3. Range component (measures the dynamic range)
        embedding_range = np.ptp(embedding_np)  # Peak-to-peak (max - min)
        range_component = min(1.0, embedding_range / 6.0)  # Normalize by expected range

        # Combine components with weights
        entropy = 0.4 * variance_component + 0.4 * dist_component + 0.2 * range_component

        # Ensure we return a reasonable value between 0.1 and 1.0
        # This prevents the entropy from being too close to zero
    ...
    def get_dynamic_compression_dim(self, embedding, base_ratio=0.6, min_ratio=0.4, max_ratio=0.8):
        ...
        # Scale compression ratio based on entropy with more meaningful mapping
        # Higher entropy (more information) = less compression (preserve more dimensions)
        # Lower entropy (less information) = more compression (compress more aggressively)

        # Invert the relationship (higher entropy → higher ratio → more dimensions preserved)
        # This ensures content with higher information density keeps more dimensions
        dynamic_ratio = min_ratio + (entropy * (max_ratio - min_ratio))
...
class EnhancedEmbeddingVAE(nn.Module):
    ...
    def calculate_semantic_entropy(self, embedding):
        ...
        # Use encoder attention to analyze importance of different dimensions
        if isinstance(embedding, torch.Tensor):
            with torch.no_grad():
                ...
                res1_out = self.encoder_res1(embedding)
                x_tmp = embedding.expand(-1, self.hidden_dim)
                x_tmp = x_tmp + res1_out
                attention_weights = self.encoder_attention(x_tmp)

                # Measure concentration of attention
                attention_np = attention_weights.cpu().numpy()
                attention_entropy = -np.sum(attention_np * np.log(attention_np + 1e-10)) / np.log(attention_np.shape[1])

                # Combine with variance-based entropy (weighted average)
                entropy = 0.7 * min(1.0, variance / 10.0) + 0.3 * attention_entropy
        else:
            # Fallback to simple variance-based entropy
            entropy = min(1.0, variance / 10.0)
...
def compress_embeddings_vae(embedding, vae_compressor, use_dynamic=True):
    """
    ...
    with torch.no_grad():
        if use_dynamic:
            compressed = vae_compressor.compress_with_dynamic_ratio(embedding_tensor)
        else:
