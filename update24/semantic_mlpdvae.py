...
class MLPDenoisingVAE(nn.Module):
    ...
    def __init__(self, input_dim, hidden_dim, latent_dim, dropout=0.1):
        ...
        # Add context scoring and fusion networks
    ...
    def update_context(self, embedding):
        """Add new embedding to context window"""
        if isinstance(embedding, torch.Tensor):
            embedding = embedding.detach().cpu().numpy()
        self.context_window.append(embedding)
    ...
    def decode(self, z, apply_kb=True):
        ...
        if apply_kb and hasattr(self, 'use_kb') and self.use_kb:
            try:
                kb = get_or_create_knowledge_base()

                # Process each item in batch
                for i in range(reconstructed.shape[0]):
                    # Apply embedding-level enhancements
                    if hasattr(kb, 'enhance_embedding'):
                        orig_vec = reconstructed[i].cpu().detach().numpy()
                        enhanced_vec = kb.enhance_embedding(orig_vec, None)

                        # Create blend tensor
                        enhanced_tensor = torch.tensor(enhanced_vec,
                                                       device=reconstructed.device,
                                                       dtype=reconstructed.dtype)

                        # Apply weighted blend
                        reconstructed[i] = 0.7 * reconstructed[i] + 0.3 * enhanced_tensor
            except Exception as e:
                logger.debug(f"KB enhancement in decoder failed: {e}")
    ...
    def decode_with_text_guidance(self, z, text_hint=None, text_context=None, text_contexts=None):
        ...
        try:
            ...
            if hasattr(self, 'get_text_embedding'):
                ...
                # Ensure proper shape
                if len(hint_tensor.shape) == 1:
                    hint_tensor = hint_tensor.unsqueeze(0)

                # Ensure reconstructed has proper shape for comparison
                recon_tensor = reconstructed
                if len(recon_tensor.shape) == 1:
                ...
                if (text_context is not None) or (text_contexts is not None and len(text_contexts) > 0):
                    hint_weight += 0.05

                # Apply the hint with weight
                if len(hint_tensor.shape) == 2 and hint_tensor.shape[0] == 1:
                    hint_tensor = hint_tensor.squeeze(0)

                adjusted = reconstructed + hint_weight * hint_tensor
...
class KnowledgeEnhancedDecoder(nn.Module):
    """
    Enhanced decoder that integrates knowledge base information and context
    for better semantic reconstruction.
    """

    def __init__(self, input_dim, hidden_dim, kb=None):
        super().__init__()
        ...
        self.context_memory_size = 16
        self.context_dim = input_dim
        self.context_memory = nn.Parameter(torch.randn(self.context_memory_size, self.context_dim))
        ...
        # KB integration network
        self.kb_integration = nn.Sequential(
    ...
    def update_context_memory(self, embedding):
        """
        Update context memory with new embedding using attention-based update.

        Args:
            embedding: New embedding to add to context
        """
        ...
        # Calculate attention scores with current memory
        combined = torch.cat([
            embedding.expand(self.context_memory_size, -1),
            self.context_memory
        ], dim=1)

        # Get attention weights
        attention = self.context_attention(combined)

        # Update memory with attention-weighted combination
        self.context_memory.data = (1 - attention) * self.context_memory + attention * embedding

    def forward(self, z, apply_kb=True):
        """
        Forward pass with context memory and KB integration.

        Args:
            z: Input latent representation
            apply_kb: Whether to apply KB enhancement
        """
        # Ensure proper shape
        if len(z.shape) == 1:
            z = z.unsqueeze(0)
        ...
        context_scores = torch.matmul(z, self.context_memory.t())
        context_weights = F.softmax(context_scores, dim=1)

        # Get weighted context
        context_vector = torch.matmul(context_weights, self.context_memory)

        # Basic decoding
        decoded = self.decoder(z)

        # Apply KB enhancement if requested
        if apply_kb and self.kb is not None:
            try:
                # Get KB enhancement for each item in batch
                kb_enhanced = []
                for i in range(z.shape[0]):
                    if hasattr(self.kb, 'enhance_embedding'):
                        # Get embedding enhancement from KB
                        orig_vec = decoded[i].cpu().detach().numpy()
                        enhanced_vec = self.kb.enhance_embedding(orig_vec, None)

                        # Convert back to tensor
                        enhanced_tensor = torch.tensor(enhanced_vec, dtype=torch.float32).to(z.device)
                        kb_enhanced.append(enhanced_tensor)

                if kb_enhanced:
                    # Stack enhanced embeddings
                    kb_tensor = torch.stack(kb_enhanced)

                    # Combine with original with learned integration
                    combined = torch.cat([decoded, kb_tensor], dim=1)
                    integrated = self.kb_integration(combined)

                    # Weighted combination
                    decoded = 0.7 * decoded + 0.3 * integrated
            except Exception as e:
                # Continue with basic decoding on error
                pass

        # Add context influence
        context_factor = 0.1
        final_output = decoded + context_factor * context_vector
...
class EnhancedMLPDenoisingVAE(MLPDenoisingVAE):
    ...
    def __init__(self, input_dim, hidden_dim, latent_dim, dropout=0.1, use_kb=True):
        ...
        # Enhanced context memory with multi-level representation
        self.context_memory_size = 32
        self.context_memory = nn.Parameter(torch.randn(self.context_memory_size, latent_dim))

        # Add dynamic context window
        self.max_context_window = 5
        self.context_window = deque(maxlen=self.max_context_window)

        # Context fusion module for integrating multiple context items
        self.context_fusion = nn.Sequential(
            nn.Linear(latent_dim * 2, latent_dim),
            nn.LayerNorm(latent_dim),
            nn.LeakyReLU(0.2),
            ...
            try:
                ...
                for i in range(reconstructed.shape[0]):
                    ...
                    if hasattr(kb, 'enhance_embedding'):
                        ...
                        # Apply weighted blend - 70% original, 30% KB enhanced
                        reconstructed[i] = 0.7 * reconstructed[i] + 0.3 * enhanced_tensor
            except Exception as e:
                logger.debug(f"KB enhancement in decoder failed: {e}")
...
def load_or_train_enhanced_mlp_dvae(model_path="enhanced_mlp_dvae_model.pth",
                                    ...
                                    use_semantic_loss=True,
                                    input_dim=None):  # Add input_dim parameter
    """
    ...
    """
    # Detect embedding dimensions or use provided dimension
    if input_dim is not None:
        # Use provided input dimension
        logger.info(f"Using provided input dimension: {input_dim}")
        hidden_dim = min(1024, input_dim * 2)
        latent_dim = min(512, input_dim)
    else:
        # Try using utility function
        try:
