...
class EmbeddingCompressorVAE(nn.Module):
    ...
    def get_dynamic_compression_dim(self, embedding, base_ratio=0.6, min_ratio=0.4, max_ratio=0.85, text=None):
        ...
        # Raise minimum ratio when entropy is high (high corruption risk)
        floor_ratio = min_ratio
        if entropy > 0.7:  # "highly informative" → more dims
            floor_ratio = min(min_ratio * 1.5, (min_ratio + max_ratio) / 2)  # Capped to avoid exceeding max_ratio
            logger.debug(f"Raising minimum ratio floor from {min_ratio:.4f} to {floor_ratio:.4f} due to high entropy")

        # Calculate final ratio with more dynamic range - now using floor_ratio
        dynamic_ratio = floor_ratio + entropy_factor * (max_ratio - floor_ratio)
        ...
        # Clamp to range
        dynamic_ratio = max(floor_ratio, min(max_ratio, dynamic_ratio))
...
class EnhancedEmbeddingVAE(nn.Module):
    ...
    def get_dynamic_compression_dim(self, embedding, base_ratio=0.6, min_ratio=0.4, max_ratio=0.8):
        ...
        dynamic_ratio = floor_ratio + (entropy * (max_ratio - floor_ratio))

        # Log the calculation details
        logger.debug(
            f"Dynamic ratio calculation: {floor_ratio} + ({entropy} * ({max_ratio} - {floor_ratio})) = {dynamic_ratio}")
