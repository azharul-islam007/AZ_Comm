...
class PhysicalChannelLayer:
    ...
    def _apply_channel_effects(self, signal):
        ...
        # ============ FIX: Correct SNR estimation ============
        # Use the configured SNR directly instead of estimating from noise
        # The configured SNR is the ground truth for our experiments
        self.channel_stats['estimated_snr'] = self.snr_db  # Use configured SNR

        # Optional: Calculate actual received SNR for logging (but don't use for adaptation)
        actual_noise_power = np.mean(np.abs(received - signal) ** 2)
        actual_snr = 10 * np.log10(signal_power / max(actual_noise_power, 1e-10))

        # Log both values for debugging
        if self.channel_stats['transmission_count'] % 10 == 0:
            logger.debug(f"Configured SNR: {self.snr_db:.1f}dB, Actual received SNR: {actual_snr:.1f}dB")
        # ============ END FIX ============

        self.channel_stats['transmission_count'] += 1

        # Store channel state periodically
