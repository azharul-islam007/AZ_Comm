...
class EnhancedPhysicalSemanticIntegration:
    ...
    def transmit(self, embedding, debug=False, use_kb=True, context=None):
        """Enhanced transmit with semantic anchor detection"""
        ...
        if anchor_weights is not None:
            ...
            importance_weights = np.clip(importance_weights, 0.1, 1.0)  # Ensure reasonable range
        else:
            importance_weights = base_weights

        # Use the enhanced transmit method if available
        if hasattr(self._physical_channel, 'transmit') and hasattr(self._physical_channel, '_smart_arq_decision'):
