...
# Import Smart-ARQ performance tracking
try:
    from smart_arq_performance import integrate_smart_arq_logging, smart_arq_tracker
except ImportError:
    integrate_smart_arq_logging = None
    smart_arq_tracker = None
...
def run_enhanced_pipeline(num_samples=None, noise_level=None, noise_type=None,
    ...
    if ENABLE_PHYSICAL_CHANNEL and physical_channel_imported:
        try:
            # Import and integrate Smart-ARQ logging
            from smart_arq_performance import integrate_smart_arq_logging, smart_arq_tracker

            # Reset tracker for this run
            smart_arq_tracker.reset_stats()

            # Check if we have a physical channel to integrate with
            if hasattr(physical_semantic_bridge, '_physical_channel') and physical_semantic_bridge._physical_channel:
                # Integrate logging into physical channel
                physical_semantic_bridge._physical_channel = integrate_smart_arq_logging(
                    physical_semantic_bridge._physical_channel)
                logger.info("Smart-ARQ performance tracking integrated into physical channel")

                # Verify integration
                if hasattr(physical_semantic_bridge._physical_channel, 'get_smart_arq_performance'):
                    logger.info("Smart-ARQ integration verified")
                else:
                    logger.warning("Smart-ARQ integration may not be complete")
            else:
                logger.warning("No physical channel available for Smart-ARQ integration")
        except Exception as e:
            logger.warning(f"Smart-ARQ performance module error: {e}")
            import traceback
            traceback.print_exc()
            ...
            if ENABLE_PHYSICAL_CHANNEL and physical_channel_imported:
                ...
                if ENABLE_PHYSICAL_CHANNEL and physical_channel_imported:
                    if hasattr(physical_semantic_bridge._physical_channel, '_last_retransmission_count'):
                        ...
                        if retrans_count > 0:
                            sample_result["smart_arq_retransmissions"] = retrans_count
                            logger.debug(f"Sample {i}: Smart-ARQ used {retrans_count} retransmissions")
    ...
    if ENABLE_PHYSICAL_CHANNEL and physical_channel_imported:
        try:
            ...
            from smart_arq_performance import smart_arq_tracker
            ...
            if smart_arq_summary['overview']['total_transmissions'] > 0:
                logger.info(f"Total Transmissions: {smart_arq_summary['overview']['total_transmissions']}")
                logger.info(f"Total Retransmissions: {smart_arq_summary['overview']['total_retransmissions']}")
                logger.info(
                    f"Average Retransmissions/Frame: {smart_arq_summary['overview']['avg_retransmissions_per_frame']:.3f}")
                logger.info(f"Retransmission Rate: {smart_arq_summary['overview']['retransmission_rate'] * 100:.1f}%")
                ...
                logger.info(
                    f"\nSemantic Preservation Rate: {smart_arq_summary['semantic_performance']['semantic_preservation_rate'] * 100:.1f}%")
                logger.info(f"Average Overhead: {smart_arq_summary['efficiency']['avg_overhead_ms']:.2f} ms")

                # Save detailed log
                smart_arq_log_path = os.path.join(run_dir, "smart_arq_performance.json")
                smart_arq_tracker.log_to_file(smart_arq_log_path)
                logger.info(f"Detailed Smart-ARQ log saved to {smart_arq_log_path}")
            else:
                logger.info("No Smart-ARQ transmissions recorded")

        except Exception as e:
            logger.warning(f"Could not collect Smart-ARQ performance data: {e}")
    if ENABLE_PHYSICAL_CHANNEL and physical_channel_imported:
        try:
    ...
    with open(os.path.join(run_dir, "summary.txt"), "w") as f:
        ...
        if "smart_arq_performance" in results:
            ...
            f.write(
                f"Average Retransmissions per Frame: {arq_perf['overview']['avg_retransmissions_per_frame']:.3f}\n")
    ...
    if "smart_arq_performance" in results:
        arq_perf = results["smart_arq_performance"]
        total_retrans = arq_perf['overview']['total_retransmissions']
        if total_retrans > 0:
