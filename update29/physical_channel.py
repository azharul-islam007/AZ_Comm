...
class PhysicalChannelLayer:
    ...
    def adapt_to_channel_conditions(self, estimated_snr=None):
        ...
        # Determine the SNR to use for adaptation
        if estimated_snr is not None:
            snr = estimated_snr
        else:
            # Use channel statistics if available
            snr = self.channel_stats.get('estimated_snr', self.snr_db)

        # For frequency-selective channels, apply a fading margin
        if self.channel_type == 'frequency_selective':
            # Check if we have very poor channel conditions
            if snr < 5.0:
                # Use a floor to prevent extreme parameters
                snr = max(snr, 5.0)
                logger.debug(f"Frequency-selective channel: Flooring SNR at {snr:.1f} dB")

            # Apply a small margin for stability
            margin = 2.0  # dB margin for frequency-selective fading
            snr = snr - margin

        # For other fading channels, apply appropriate margins
        elif self.channel_type == 'rayleigh':
            margin = 3.0  # Larger margin for Rayleigh fading
            snr = snr - margin
        elif self.channel_type == 'rician':
            margin = 1.5  # Smaller margin for Rician (has LOS component)
            snr = snr - margin
        ...
        qam16_threshold = config.get("physical.mod_thresholds.16QAM", 18.0)
        qpsk_threshold = config.get("physical.mod_thresholds.QPSK", 10.0)
        bpsk_threshold = config.get("physical.mod_thresholds.BPSK", 5.0)

        # Store previous modulation for logging
        previous_modulation = self.modulation_order
        previous_coding_rate = self.coding_rate
        ...
        elif snr > bpsk_threshold:  # Poor conditions
            new_modulation_order = 2  # BPSK
            modulation_name = "BPSK"
        else:  # Very poor conditions
            # Use BPSK with maximum protection
            new_modulation_order = 2
            new_coding_rate = 0.2
            modulation_name = "BPSK"
        ...
        # Determine channel condition description
        if snr > qam16_threshold:
            condition = "excellent" if snr > qam64_threshold else "good"
        elif snr > qpsk_threshold:
            condition = "fair"
        else:
            condition = "poor"

        # Log adaptation if parameters changed
        if (self.modulation_order != previous_modulation or
                abs(self.coding_rate - previous_coding_rate) > 0.01):
            logger.info(f"Channel conditions {condition} (SNR: {snr:.1f}dB): "
                        f"Using {modulation_name} with coding rate {new_coding_rate}")

        # Store adaptation history
        self.channel_stats['adaptation_history'] = {
            'snr': snr,
            'modulation': modulation_name,
            'coding_rate': new_coding_rate,
            'condition': condition,
            'timestamp': self.channel_stats['transmission_count']
        }
    ...
    def _apply_channel_effects(self, signal):
        ...
        # Initialize channel gain for tracking
        channel_gain = 1.0

        # Apply channel effects based on channel type
        if self.channel_type == 'awgn':
            # Just add noise for AWGN channel
            received = signal + noise
            effective_snr = self.snr_db  # No channel loss
            ...
            # Apply fading
            channel_output = h * signal

            # Calculate channel gain
            channel_gain = np.mean(np.abs(h) ** 2)

            # Add noise
            received = channel_output + noise

            # Calculate effective SNR
            effective_snr = 10 * np.log10(channel_gain * signal_power / noise_power)
            ...
            # Use configured tap power profile if available
            try:
                # Try to get tap power profile from config
                from config_manager import ConfigManager
                config_manager = ConfigManager()
                tap_powers = config_manager.get("physical.tap_power_profile", [0.8, 0.6, 0.4, 0.2, 0.1])
                num_taps = config_manager.get("physical.multipath_taps", 5)
            except:
                # Default values if config not available
                tap_powers = [0.8, 0.6, 0.4, 0.2, 0.1]
                num_taps = 5

            # Ensure tap_powers has correct length
            if len(tap_powers) != num_taps:
                # Pad with zeros or truncate
                if len(tap_powers) < num_taps:
                    tap_powers.extend([0.1] * (num_taps - len(tap_powers)))
                else:
                    tap_powers = tap_powers[:num_taps]

            # Convert to numpy array and normalize
            tap_powers = np.array(tap_powers, dtype=float)
            tap_powers = tap_powers / np.sum(tap_powers)

            # Generate channel taps with proper power distribution
            taps = np.zeros(num_taps, dtype=complex)

            for i in range(num_taps):
                # Each tap is Rayleigh distributed with specified power
                # Using proper normalization for complex Gaussian
                taps[i] = np.sqrt(tap_powers[i] / 2) * (np.random.randn() + 1j * np.random.randn())

            # Apply multipath channel through convolution
            from scipy import signal as sp_signal
            channel_output = sp_signal.convolve(signal, taps, mode='same')

            # Calculate actual channel power gain
            # This is the average power gain of the channel
            channel_power = np.mean(np.abs(channel_output) ** 2)
            channel_gain = channel_power / signal_power

            # Add noise
            received = channel_output + noise

            # Calculate effective SNR accounting for channel gain
            # The effective SNR is reduced by the channel loss
            effective_snr = 10 * np.log10(channel_gain * signal_power / noise_power)

            # Store additional channel statistics for frequency-selective channel
            self.channel_stats['channel_taps'] = taps
            self.channel_stats['channel_gain'] = channel_gain
        ...
        # Update channel statistics with effective SNR
        self.channel_stats['estimated_snr'] = effective_snr
        self.channel_stats['transmission_count'] += 1
        self.channel_stats['channel_type'] = self.channel_type
        self.channel_stats['configured_snr'] = self.snr_db
        self.channel_stats['channel_gain'] = channel_gain

        # Calculate instantaneous BER estimate (simplified)
        # This is a rough estimate based on the effective SNR
        if effective_snr > 20:
            estimated_ber = 1e-6
        ...
        self.channel_stats['estimated_ber'] = estimated_ber

        # Log channel conditions periodically
        if self.channel_stats['transmission_count'] % 50 == 0:
            logger.info(f"Channel stats after {self.channel_stats['transmission_count']} transmissions:")
            logger.info(f"  Channel type: {self.channel_type}")
            logger.info(f"  Configured SNR: {self.snr_db:.1f} dB")
            logger.info(f"  Effective SNR: {effective_snr:.1f} dB")
            logger.info(f"  Channel gain: {channel_gain:.3f}")
            logger.info(f"  Estimated BER: {estimated_ber:.2e}")
    ...
    def _save_channel_state(self):
        ...
        try:
            ...
            save_data = {
                'transmission_count': self.channel_stats['transmission_count'],
                'channel_type': self.channel_type,
                'configured_snr': self.snr_db,
                ...
                'channel_gain': self.channel_stats.get('channel_gain', 1.0),
                'estimated_ber': self.channel_stats.get('estimated_ber', 0.0)
            }

            # Add channel-specific data
            if self.channel_type == 'frequency_selective' and 'channel_taps' in self.channel_stats:
                save_data['channel_taps'] = self.channel_stats['channel_taps']

            # Add adaptation history if available
            if 'adaptation_history' in self.channel_stats:
                save_data['adaptation_history'] = self.channel_stats['adaptation_history']
    ...
    def _estimate_error_rate(self, original_bits, received_bits):
        ...
        # Update channel statistics with exponential moving average
        alpha = 0.1  # Smoothing factor
        if 'error_rate' not in self.channel_stats:
            self.channel_stats['error_rate'] = error_rate
        else:
            self.channel_stats['error_rate'] = (1 - alpha) * self.channel_stats['error_rate'] + alpha * error_rate

        # Track error rate history
        if 'error_rate_history' not in self.channel_stats:
            self.channel_stats['error_rate_history'] = []

        self.channel_stats['error_rate_history'].append({
            'ber': error_rate,
            'snr': self.channel_stats['estimated_snr'],
            'modulation': self.modulation_order,
            'coding_rate': self.coding_rate
        })

        # Keep only last 100 entries
        if len(self.channel_stats['error_rate_history']) > 100:
            self.channel_stats['error_rate_history'] = self.channel_stats['error_rate_history'][-100:]

        return error_rate

    def get_channel_statistics(self):
        """
        Get comprehensive channel statistics for monitoring and debugging.
        """
        stats = {
            'channel_type': self.channel_type,
            'configured_snr_db': self.snr_db,
            'estimated_snr_db': self.channel_stats.get('estimated_snr', self.snr_db),
            'channel_gain': self.channel_stats.get('channel_gain', 1.0),
            'transmission_count': self.channel_stats.get('transmission_count', 0),
            'average_ber': self.channel_stats.get('error_rate', 0.0),
            'current_modulation': f"{self.modulation}-{self.modulation_order}",
            'current_coding_rate': self.coding_rate,
            'adaptive_modulation_enabled': self.enable_adaptive_modulation,
            'unequal_error_protection_enabled': self.enable_unequal_error_protection
        }

        # Add recent adaptation history if available
        if 'adaptation_history' in self.channel_stats:
            stats['last_adaptation'] = self.channel_stats['adaptation_history']

        # Add error rate trend if available
        if 'error_rate_history' in self.channel_stats and len(self.channel_stats['error_rate_history']) > 0:
            recent_errors = [h['ber'] for h in self.channel_stats['error_rate_history'][-10:]]
            stats['recent_ber_trend'] = {
