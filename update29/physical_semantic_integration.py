    ...
    def transmit(self, embedding, debug=False, use_kb=True, context=None):
        ...
        if hasattr(self._physical_channel, 'transmit'):
            ...
            if hasattr(self._physical_channel, '_smart_arq_decision'):
                ...
                # Check if the transmit method accepts max_retransmissions parameter
                import inspect
                try:
                    sig = inspect.signature(self._physical_channel.transmit)
                    if 'max_retransmissions' in sig.parameters:
                        # Method supports max_retransmissions
                        ...
                        # Method doesn't support max_retransmissions, use standard call
                        logger.debug("Physical channel transmit doesn't support max_retransmissions parameter")
                        return self._physical_channel.transmit(embedding, importance_weights, debug)
                except Exception as e:
                    logger.debug(f"Error checking transmit signature: {e}")
