...
def visualize_system_performance(results, save_dir="./sem_com_result", run_id=None):
    ...
    # Add this new section after the existing visualizations in visualize_system_performance function

    # ===== 8. Channel Conditions and UEP Analysis =====
    if "physical_channel_enabled" in results["settings"] and results["settings"]["physical_channel_enabled"]:
        plt.figure(figsize=(16, 12))
        ...
        if channel_stats['snr_evolution']:
            ...
            plt.axhline(y=25, color='g', linestyle='--', alpha=0.5, label='Excellent threshold')
            plt.axhline(y=18, color='y', linestyle='--', alpha=0.5, label='Good threshold')
            plt.axhline(y=10, color='orange', linestyle='--', alpha=0.5, label='Fair threshold')
            plt.axhline(y=5, color='r', linestyle='--', alpha=0.5, label='Poor threshold')
            ...
            plt.ylabel('SNR (dB)')
            plt.title('Channel SNR Evolution')
            plt.legend(loc='best')
        ...
        if channel_stats['modulation_orders'] and channel_stats['coding_rates']:
            ...
            ax1.set_ylabel('Modulation Order', color='b')
            ax1.tick_params(axis='y', labelcolor='b')
            ax1.set_ylim(0, 70)
            ax1.set_yticks([2, 4, 16, 64])
            ax1.set_yticklabels(['BPSK', 'QPSK', '16-QAM', '64-QAM'])

            # Plot coding rate
            ax2.step(x_axis, channel_stats['coding_rates'], 'r-', where='mid',
                     linewidth=2, label='Coding Rate')
            ax2.set_ylabel('Coding Rate', color='r')
            ax2.tick_params(axis='y', labelcolor='r')
            ax2.set_ylim(0, 1.0)

            plt.title('Adaptive Modulation and Coding')
        ...
        if sum(protection_level_counts.values()) > 0:
            ...
            plt.title('Unequal Error Protection (UEP) Distribution')
        ...
        # Subplot 4: Channel Performance Summary
        plt.subplot(2, 2, 4)
        plt.axis('off')
        ...
        # Channel type from settings
        channel_type = results["settings"].get("physical_channel", {}).get("channel_type", "Unknown")

        summary_text = f"Channel Performance Summary\n\n"
        summary_text += f"Channel Type: {channel_type.upper()}\n"
        summary_text += f"Samples Processed: {len(results['samples'])}\n\n"

        summary_text += f"SNR Statistics:\n"
        summary_text += f"  Average: {avg_snr:.1f} dB\n"
        summary_text += f"  Std Dev: {std_snr:.1f} dB\n"
        summary_text += f"  Range: [{min_snr:.1f}, {max_snr:.1f}] dB\n\n"

        summary_text += f"Average Channel Gain: {avg_gain:.3f}\n\n"

        summary_text += f"Modulation Usage:\n"
        for mod_name, count in sorted(mod_usage.items()):
            percentage = (count / len(channel_stats['modulation_orders']) * 100) if channel_stats[
                'modulation_orders'] else 0
            summary_text += f"  {mod_name}: {count} ({percentage:.1f}%)\n"

        summary_text += f"\nAdaptive Features:\n"
        summary_text += f"  Adaptive Modulation: {'Enabled' if results['settings'].get('physical_channel', {}).get('enable_adaptive_modulation', True) else 'Disabled'}\n"
        summary_text += f"  Unequal Error Protection: {'Enabled' if results['settings'].get('physical_channel', {}).get('enable_unequal_error_protection', True) else 'Disabled'}\n"
        summary_text += f"  Content-Adaptive Coding: {'Enabled' if results['settings'].get('physical_channel', {}).get('enable_content_adaptive_coding', True) else 'Disabled'}\n"

        plt.text(0.05, 0.95, summary_text, verticalalignment='top', fontsize=10,
