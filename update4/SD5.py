...
class AdvancedRLAgent(nn.Module):
    ...
    def select_action(self, state, budget_remaining, force_basic=False):
        """Select action using policy with budget awareness"""
...
def run_enhanced_pipeline(num_samples=None, noise_level=None, noise_type=None,
    ...
    # Output model dimensions for debugging
    if dvae is not None:
        logger.info(
            f"[PIPELINE] Model dimensions: input={dvae.input_dim}, hidden={dvae.hidden_dim}, latent={dvae.latent_dim}")
    else:
    ...
    # Log RL agent status
    if use_rl:
        logger.info(f"[PIPELINE] Using enhanced RL agent for API optimization (exploration rate: {rl_agent.epsilon:.2f})")
    else:
    ...
    for i, (sentence, embedding) in enumerate(
        ...
        try:
            ...
            if use_vae_compression and vae_compressor:
                ...
                target_dim = 460  # The dimension the VAE was trained on
            ...
            with torch.no_grad():
                ...
                embedding_tensor = ensure_tensor_shape(embedding_tensor, expected_dim=2, target_feature_dim=dvae.input_dim)
                logger.info(f"[PIPELINE] Embedding shape: {embedding_tensor.shape}, Model input dim: {dvae.input_dim}")
            ...
            if use_knowledge_base and kb_working and semantic_reconstructed != corrupted_text:
                kb = get_or_create_knowledge_base()
    ...
    if use_rl:
        results["rl_metrics"] = {
            ...
            "exploration_rate": rl_agent.epsilon,  # Changed from exploration_rate to epsilon
            "q_table": rl_agent.policy_net.state_dict(),  # Changed from q_table.tolist()
            "num_states": rl_agent.state_dim,  # Changed from num_states
    ...
    try:
        ...
        # Evaluate all samples with enhanced metrics
        enhanced_metrics = evaluation_framework.evaluate_reconstruction(
...
def process_sample_with_vae(embedding, vae_compressor):
    ...
    target_dim = 460  # Hardcode the expected dimension
