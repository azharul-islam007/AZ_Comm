...
class ContentClassifier(nn.Module):
    ...
    def classify(self, embedding):
        ...
        # Check if we have rule-based model (no neural network)
        if not hasattr(self, 'network') or self.network is None:
            # Simple fallback using embedding statistics
            ...
            if std_val > 0.3 * mean_val:  # High variance
                class_idx = 3  # argumentative
            elif max_val > 3 * mean_val:  # High peaks
                class_idx = 1  # legislative
            elif mean_val > 0.5:  # High overall values
                class_idx = 2  # factual
            else:
                class_idx = 0  # procedural

            # Generate fake probabilities
            probs = [0.1, 0.1, 0.1, 0.1]
            probs[class_idx] = 0.7

            return self.content_classes[class_idx], probs

        # Original neural network-based classification
...
# In content_adaptive_coding.py
def create_simple_content_classifier(save_path='./models/content_classifier.pth'):
    """
    Create a simple rule-based content classifier as a fallback when ML model is not available.
    """
    logger.info("Creating simple rule-based content classifier")

    # Create directory if it doesn't exist
    os.makedirs(os.path.dirname(save_path), exist_ok=True)

    # Define keyword lists for each content type
    content_keywords = {
        'procedural': [
            'agenda', 'vote', 'voting', 'session', 'president', 'chair', 'floor',
            'point', 'order', 'rule', 'procedure', 'adjourn', 'convene', 'meeting'
        ],
        'legislative': [
            'directive', 'regulation', 'proposal', 'amendment', 'committee',
            'report', 'legislation', 'law', 'legal', 'treaty', 'article',
            'provision', 'framework', 'draft'
        ],
        'factual': [
            'statistics', 'data', 'figures', 'study', 'research', 'analysis',
            'report', 'survey', 'findings', 'evidence', 'results', 'information',
            'experts', 'documents', 'published'
        ],
        'argumentative': [
            'argue', 'believe', 'consider', 'opinion', 'view', 'position',
            'debate', 'disagree', 'oppose', 'support', 'favor', 'against',
            'however', 'nevertheless', 'despite', 'contrary', 'whereas'
        ]
    }

    # Save this simple model as a JSON file
    model_data = {
        'type': 'rule_based',
        'content_classes': list(content_keywords.keys()),
        'keywords': content_keywords,
        'embedding_dim': 460  # Match your current embedding dimension
    }

    # Save as JSON since it's not a torch model
    with open(save_path.replace('.pth', '.json'), 'w') as f:
        json.dump(model_data, f, indent=2)

    # Also create a dummy .pth file for compatibility
    dummy_state = {
        'model_state_dict': None,
        'embedding_dim': 460,
        'num_classes': len(content_keywords),
        'content_classes': list(content_keywords.keys())
    }
    torch.save(dummy_state, save_path)

    logger.info(f"Simple content classifier saved to {save_path}")
    return True
...
class ContentAdaptivePhysicalChannel(PhysicalChannelLayer):
    ...
    def _load_content_classifier(self):
        """Load the content classifier model if available"""
        try:
            if os.path.exists(self.content_classifier_path):
                ...
                logger.warning(f"Content classifier not found at {self.content_classifier_path}")
                logger.info("Will use default protection strategies")
                return False
