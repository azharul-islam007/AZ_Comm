    ...
    def classify(self, embedding):
        ...
        try:
            ...
            if current_dim != expected_dim:
                ...
                if current_dim < expected_dim:
                    # Pad with zeros
                    padding = torch.zeros(embedding_tensor.shape[0], expected_dim - current_dim,
...
class ContentAdaptivePhysicalChannel(PhysicalChannelLayer):
    ...
    def _load_content_classifier(self):
        ...
        try:
            # First, determine the correct embedding dimension
            compressed_dim = None
            if hasattr(self, 'vae_compression') and self.vae_compression:
                try:
                    # Try to get compressed dimension from configuration
                    from compression_vae import VAE_COMPRESSION_FACTOR
                    # Default BERT dimension is 768, apply compression factor
                    compressed_dim = int(768 * VAE_COMPRESSION_FACTOR)
                    logger.info(f"Detected VAE compressed dimension: {compressed_dim}")
                except ImportError:
                    logger.warning("Could not import VAE_COMPRESSION_FACTOR")

            # Use detected dimension or fallback to default
            embedding_dim = compressed_dim if compressed_dim is not None else 768
            ...
            if os.path.exists(self.content_classifier_path):
                ...
                if not isinstance(checkpoint, dict) or 'model_state_dict' not in checkpoint:
                    ...
                    # Train a classifier with correct dimensions
                    return self._train_fallback_classifier(embedding_dim)
                ...
                try:
                    ...
                    logger.info(f"Content classifier loaded successfully with dimension {embedding_dim}")
                    return True
                except Exception as e:
                    logger.warning(f"Error loading classifier weights: {e}")
                    # Continue with newly initialized classifier
                    return True
            else:
                # Create a classifier with correct dimensions
                return self._train_fallback_classifier(embedding_dim)
        except Exception as e:
