...
def transmit_through_physical_channel(embedding, importance_weights=None, debug=False, use_kb=True, context=None,
    ...
    Args:
        ...
        context: Primary context for reconstruction (new parameter)
        context_list: List of additional context items (new parameter)
    """
    ...
    if use_kb:
        try:
            ...
            if hasattr(kb, 'enhance_embedding'):
                ...
                # Then apply context enhancement if we have context available
                if context and hasattr(kb, 'enhance_with_context'):
                    # Use primary context
                    enhanced_with_context = kb.enhance_with_context(enhanced, context)

                    # Apply additional contexts if available
                    if context_list and len(context_list) > 0 and hasattr(kb, 'enhance_with_context'):
                        # Get additional contexts (skip the first one which is the primary context)
                        additional_contexts = context_list
                        # Apply them one by one
                        for add_ctx in additional_contexts:
                            if add_ctx != context:  # Avoid duplicate application
                                enhanced_with_context = kb.enhance_with_context(enhanced_with_context, add_ctx)

                    # Use the context-enhanced version
                    enhanced = enhanced_with_context

                    # Apply a dynamic blend factor based on context availability
                    ...
                    # No context-enhancement available, use standard blend factor
                    blend_factor = 0.2

                # Apply the blending
                received_embedding = (1.0 - blend_factor) * received_embedding + blend_factor * enhanced
                logger.debug(f"Applied post-transmission KB enhancement" +
                             (
                                 f" with {1 + (len(context_list) if context_list else 0)} contexts" if context_list else ""))
        except Exception as e:
